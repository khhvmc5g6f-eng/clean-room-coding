...
3. **Licence discovery** -- `cleanroom licence <path>` (usually `zone-r`).
...
4. **Jurisdiction** -- `cleanroom jurisdiction` builds `JURISDICTION_MATRIX.json`
...
5. **Analyse** (Zone R, analyst agents only) -- `cleanroom analyse` writes
...
6. **Specify** -- `cleanroom specify add-requirement` / `add-behavioral` /
   `report` builds the requirement graph and GIVEN/WHEN/THEN behavioural
   tests from what phase 5 found. Only `observable_requirement` +
   contamination `C0` nodes will be eligible for handoff.

7. **Sanitise** -- `cleanroom sanitise <candidate-file>` on every document
...
8. **Handoff** -- `cleanroom handoff --specification-version vN --all-c0`
...
9. **Architect / Build** -- `cleanroom architect` records an ADR per
...
10. **Test / Compare** -- `cleanroom test` runs the behavioural suite (and
...
11. **Provenance** -- `cleanroom provenance` generates SPDX + CycloneDX
    SBOMs for Zone I's declared dependencies.

12. **Audit** -- `cleanroom audit` re-runs the isolation self-test and
    evidence-chain integrity check, plus a licence scan of Zone H (should
    only ever contain C0 material).

13. **Legal / Judge** -- `cleanroom legal --access-authority ...` runs the
...
14. **Report / Release** -- `cleanroom report --version vN` assembles
